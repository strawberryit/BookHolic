...
public class YeongcheonLibrarySearchTask extends Yes24LibrarySearchTask {

    @Getter
    final String libraryCode = "";
    static final String libraryName = "영천시립도서관";
    static final String baseUrl = "http://www.yclib.go.kr:8080";

    public YeongcheonLibrarySearchTask(MainActivity activity) {
        super(activity, libraryName, baseUrl);
    }

    @Override
    public LibrarySearchTask create() {
        SoftReference<LibrarySearchTask> ref = new SoftReference<>(new YeongcheonLibrarySearchTask(this.mActivity));
        return ref.get();
    }
