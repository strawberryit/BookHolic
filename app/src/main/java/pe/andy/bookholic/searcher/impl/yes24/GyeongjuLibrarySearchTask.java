...
public class GyeongjuLibrarySearchTask extends Yes24LibrarySearchTask {

    @Getter
    final String libraryCode = "";
    static final String libraryName = "경주시립도서관";
    static final String baseUrl = "http://library.gyeongju.go.kr:81";

    public GyeongjuLibrarySearchTask(MainActivity activity) {
        super(activity, libraryName, baseUrl);
    }

    @Override
    public LibrarySearchTask create() {
        SoftReference<LibrarySearchTask> ref = new SoftReference<>(new GyeongjuLibrarySearchTask(this.mActivity));
        return ref.get();
    }
